...
exports.register = function(commander) {
    ...
    function getRootUrl(envPort) {
        var fs = require('fs');
        var projectPath = fis.config.get('base.path') || '';
        var viewName = 'index';
        var nodePath = require('path');
        if (!fis.util.exists(nodePath.join(root, projectPath, viewName + '.html'))) {
            try {
                var reg = /\.html$/;
                var projectDir = require('path').join(root, projectPath);
        ...
        return 'http://' + getLoaclIP() + ':' + envPort + '/' +
                projectPath + '/' + viewName +
                '?uc_param_str=dsdnfrpfbivesscpgimibtbmnisieijblauputogpintnw';
    }

    function lanuch(){
        var args = [].slice.call(arguments);
        var envPort = fis.config.get('base.port') || 9000;
